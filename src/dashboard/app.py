...
if not Path("data/raw/pakistan_youth_mental_health.csv").exists():
...
<style>
    ...
    /* Sidebar widgets — dark background */
...
def load_model():
    try:
        import joblib
        mp = Path("data/models/risk_model.joblib")
...
# ── Guard ─────────────────────────────────────────────────────────────────────
if df_raw is None:
    st.error("⚠️ No data found. Run: `python -m src.ingestion.download && python -m src.processing.clean`")
    st.stop()
...
if page == "📊 Overview":
    ...
    with col_a:
        youth_mask = df["age_group"].isin(["11-12","12-13","13-14","14-15","15-16","16-17","17-18","18-24"])
        yr = df[youth_mask]["mental_health_risk_score"].mean()
        ar = df[~youth_mask]["mental_health_risk_score"].mean()
        st.markdown(f"""<div class="insight-card insight-high">
            <strong>🔴 Youth Risk Higher Than Adults</strong><br>
            Youth (11–24): <strong>{yr:.1f}</strong> vs Adults 25+: <strong>{ar:.1f}</strong>
            — a {yr-ar:.1f} point gap
    ...
    with col_b:
        ...
        if pak and usa:
            st.markdown(f"""<div class="insight-card insight-purple">
                ...
                — driven by higher anxiety rates and lower help-seeking
            </div>""", unsafe_allow_html=True)

        hr_help = df[df["high_risk"]==1]["sought_help"].mean()*100
        lr_help = df[df["high_risk"]==0]["sought_help"].mean()*100
        st.markdown(f"""<div class="insight-card insight-medium">
            <strong>🆘 Help-Seeking Gap</strong><br>
            High-risk: <strong>{hr_help:.0f}%</strong> vs Low-risk: <strong>{lr_help:.0f}%</strong> seek help.
    ...
    with c2:
        ...
        cr["color"] = cr["country"].map(COUNTRY_COLORS)
        fig = px.bar(cr, x="country", y="high_risk",
...
# ════════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown("Year-over-year changes in youth mental health indicators")
    st.markdown("---")
    ...
    fig.add_trace(go.Scatter(x=yearly["year"], y=yearly["avg_risk"],
        mode="lines+markers", line=dict(color=C["teal"], width=3),
        marker=dict(size=8), fill="tozeroy", fillcolor="rgba(20,184,166,0.1)"))
    ...
    with c2:
        ...
        fig.add_trace(go.Scatter(x=yearly["year"], y=yearly["help_rate"]*100,
            name="Help-Seeking", line=dict(color=C["green"], width=2),
            fill="tozeroy", fillcolor="rgba(16,185,129,0.1)"))
        fig.update_layout(template="plotly_white", height=300,
            yaxis_title="Rate (%)", margin=dict(t=10))
        st.plotly_chart(fig, use_container_width=True)
    ...
    fig = px.line(cy, x="year", y="mental_health_risk_score", color="country",
        ...
        labels={"mental_health_risk_score": "Avg Risk Score"},
        template="plotly_white")
...
# ════════════════════════════════════════════════════════════════════════════════
    ...
    with c1:
        ...
        fig = px.bar(age_df, x="age_group", y="mental_health_risk_score",
    ...
    with c2:
        ...
        fig = px.bar(gdf, x="gender", y="high_risk", color="gender",
            ...
            labels={"high_risk":"High Risk Rate (%)","gender":""},
            template="plotly_white")
...
# ════════════════════════════════════════════════════════════════════════════════
    ...
    with c1:
        fig = px.bar(sa, x="sleep_bucket", y="avg_risk", color="avg_risk",
            ...
            labels={"avg_risk":"Avg Risk Score","sleep_bucket":"Sleep"},
            template="plotly_white")
    ...
    with c2:
        fig = px.bar(sa, x="sleep_bucket", y="high_risk_rate", color="high_risk_rate",
            color_continuous_scale=[[0,C["teal"]],[1,C["red"]]],
            labels={"high_risk_rate":"High Risk Rate","sleep_bucket":"Sleep"},
            template="plotly_white")
        fig.update_layout(height=300, margin=dict(t=10), coloraxis_showscale=False)
        st.plotly_chart(fig, use_container_width=True)

    low_val = sa[sa["sleep_bucket"]=="<5h"]["avg_risk"].values
    high_val = sa[sa["sleep_bucket"]=="7-8h"]["avg_risk"].values
    if len(low_val) and len(high_val):
        st.info(f"💡 People sleeping <5h score **{low_val[0]:.1f}** vs **{high_val[0]:.1f}** for 7-8h — statistically significant (p<0.0001)")

    st.subheader("Social Support Level vs Risk")
    sup_order = ["Never","Rarely","Sometimes","Usually","Always"]
    sup_agg = df.groupby("social_support")["mental_health_risk_score"].mean().reset_index()
    sup_agg["order"] = sup_agg["social_support"].map({s:i for i,s in enumerate(sup_order)})
    sup_agg = sup_agg.sort_values("order")
    fig = px.line(sup_agg, x="social_support", y="mental_health_risk_score", markers=True,
...
# ════════════════════════════════════════════════════════════════════════════════
    ...
    display["avg_sleep"] = display["avg_sleep"].round(1)
    display.columns = ["Country","Records","Avg Risk","High Risk","Anxiety","Depression","Help-Seeking","Avg Sleep (h)","Avg Activity (days/wk)"]
    ...
    for i, (m, lab) in enumerate(zip(metrics, labels)):
        ...
        with col:
            vals = country_summary[["country", m]].copy()
            if m != "avg_risk":
                vals[m] = vals[m] * 100
            fig = px.bar(vals, x="country", y=m, color="country",
                color_discrete_map=COUNTRY_COLORS,
                labels={m: lab, "country": ""},
                title=lab, template="plotly_white")
...
# ════════════════════════════════════════════════════════════════════════════════
    ...
    if model is None:
        ...
        with c2:
            ...
            soc_sup  = st.selectbox("Social Support", ["Never","Rarely","Sometimes","Usually","Always"], index=2)
...
# ════════════════════════════════════════════════════════════════════════════════
    ...
    for label, grp1_name, grp2_name, col in [
        ...
        g1 = df[df[col]==grp1_name]["mental_health_risk_score"]
        g2 = df[df[col]==grp2_name]["mental_health_risk_score"]
        if len(g1) > 0 and len(g2) > 0:
            t, p = stats.ttest_ind(g1, g2)
            tests.append({
                "Test": label, "Method": "t-test",
                "Statistic": f"t={t:.3f}", "p-value": f"{p:.4f}",
                "Significant": "✅ Yes" if p < 0.05 else "❌ No",
                "Finding": f"{grp1_name} ({g1.mean():.2f}) vs {grp2_name} ({g2.mean():.2f})",
            })
    ...
    age_gs = [g for _,g in df.groupby("age_group")["mental_health_risk_score"]]
    if len(age_gs) > 1:
        f, p = stats.f_oneway(*age_gs)
        tests.append({"Test":"Age Group Differences","Method":"ANOVA",
            "Statistic":f"F={f:.3f}","p-value":f"{p:.2e}",
            "Significant":"✅ Yes" if p<0.05 else "❌ No",
            "Finding":f"{len(age_gs)} groups compared"})

    st.dataframe(pd.DataFrame(tests), use_container_width=True, hide_index=True)

    st.markdown("---")
    st.subheader("Feature Correlation Matrix")
    num_cols = ["mental_health_risk_score","sleep_hours","activity_days_per_week",
